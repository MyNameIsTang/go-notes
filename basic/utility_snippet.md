...
9. 网络和网页应用
   ...
   - 在网页应用中使用 HTML 过滤 HTML 特殊字符：`{{html .}}` 或通过一个字段`FieldName {{ .FieldName |html }}`
   - 使用缓存模板
...
10. 其他
    ...
    - 或者：
      ```
        if err != nil {
          painc("ERROR occurred: " + err.Error())
        }
      ```

11. 处于性能考虑的最佳实践和建议
    - 尽可能的使用`:=`初始化声明一个变量（在函数内部）
    - 尽可能的使用字符代替字符串
    - 尽可能的使用切片代替数组
    - 尽可能的使用数组和切片代替映射
    - 如果只想获取切片中某项值，不需要值的索引，经可能的使用 `for range`去遍历切片，这比必须查询切片中的每一个元素要快一些
